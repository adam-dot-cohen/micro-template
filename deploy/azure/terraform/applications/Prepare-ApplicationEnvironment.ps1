...
$Environments[$Environment].Regions | % { 
	...
	Write-Host "Preparing the $($Environments[$Environment].Name) ($Environment) environment for `'$ApplicationName`' in $Location"
	
	# calculate some names
	$azureLocation = $Regions[$Location].LocationName
	$locationCode = $Regions[$Location].Abbrev
	$resourceGroupName = $ExecutionContext.InvokeCommand.ExpandString("rg-$($Tenant)-$($Environment)-$($LocationCode)-$($ApplicationName)")
	$vnetName = "vnet-$($Tenant)-$($Environment)-$($LocationCode)"

	# # Get Infrastructure KeyVault for environment
    # $infraKeyVaultName =  "kv-$($Tenant)-$($Environment)-$($LocationCode)-infra")
	
    # Write-Host "Checking for Key Vault $keyVaultName in $resourceGroupName"
    # $infraKV = Get-AzKeyVault -VaultName $infraKeyVaultName -ErrorAction SilentlyContinue
	# if ($infraKV -eq $null)
	# {
		# Write-Error "Failed to locate infrastructure keyvault $infraKeyVaultName"
		# return -1
	# }
	
	# Get/Create ResourceGroup for Application
	...
	# Get/Create KeyVault (Access policy is always applied)
		$kv = New-KeyVault -Tenant $Tenant -Environment $Environment -Location $Location -Role $ApplicationName
