    ...
    public class ReadSeekableStream : Stream
    {
        ...
        public ReadSeekableStream(Stream underlyingStream, int seekBackBufferSize)
        {
            if (underlyingStream.CanSeek)
                throw new Exception($"Provided stream {underlyingStream} is already seekable.");
        ...
        {
            var copiedFromBackBufferCount = 0;
        ...
        private long SeekForward(long origOffset)
        {
            ...
            var backwardSoughtBytes = _seekBackBufferCount - _seekBackBufferIndex;
            var seekForwardInBackBuffer = (int)Math.Min(offset, backwardSoughtBytes);
