...
namespace IdentityServer4.Quickstart.UI.Device
{
    ...
    public class DeviceController : Controller
    {
        ...
        private async Task<ProcessConsentResult> ProcessConsent(DeviceAuthorizationInputModel model)
        {
            ...
            var clientRequest = await _interaction.GetAuthorizationContextAsync(model.UserCode);
            var request = clientRequest.Client;
            ...
            {
                // emit event
                await _events.RaiseAsync(new ConsentDeniedEvent(User.GetSubjectId(), request.ClientId, model.ScopesConsented));
            }
            ...
            else if (model.Button == "yes")
            {
                ...
                if (model.ScopesConsented != null && model.ScopesConsented.Any())
                {
                    ...
                    // emit event
                    await _events.RaiseAsync(new ConsentGrantedEvent(User.GetSubjectId(), request.ClientId, request.AllowedScopes, grantedConsent.ScopesValuesConsented, grantedConsent.RememberConsent));
                }
                else
                {
