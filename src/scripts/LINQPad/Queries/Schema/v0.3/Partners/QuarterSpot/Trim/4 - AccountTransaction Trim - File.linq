...
var outputFilename = $"{outputDirectory}{customerName}_Laso_R_AccountTransaction_{schemaVersion}_{asOfDate:yyyyMMdd}_{now:yyyyMMddHHmmss}.csv";

using var outputStream = File.OpenWrite(outputFilename);
using var delimitedFileWriter = new DelimitedFileWriter();
delimitedFileWriter.Open(outputStream);
delimitedFileWriter.WriteRecords<AccountTransaction_v0_3>(filteredTransactions);
